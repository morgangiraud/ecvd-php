<?php
	...
	function getPost($idPost){
		...
		try {
            $result = $conn->prepare('select * from posts where id = :idPost');
	...
	function editPost($title, $body, $image_id, $idPost, $idUser){
		global $conn;

		try{
          	$result = $conn->prepare('UPDATE posts SET title = :title, body = :body, image_id = :image_id where id = :idPost and user_id = :idUser');
			$result->bindParam(":title", $title, PDO::PARAM_STR);
            $result->bindParam(":body", $body, PDO::PARAM_STR);
            $result->bindParam(":idPost", $idPost, PDO::PARAM_INT);
